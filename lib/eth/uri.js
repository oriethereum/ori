/**
 * uri.js - ethereum uri parsing for oriethereum
 * https://github.com/oriethereum-org/ori
 */
...
/**
 * URI
 * Represents a ethereum URI.
 * @alias module:eth.URI
...
class URI {
  ...
  fromString(str, network) {
    ...
    assert(str.length > 8, 'Not a ethereum URI.');

    const prefix = str.substring(0, 8);

    assert(prefix === 'ethereum:', 'Not a ethereum URI.');
  ...
  toString() {
    ...
    if (this.amount !== -1)
      query.push(`amount=${Amount.eth(this.amount)}`);
